  ...
  override def projectSettings: Seq[Def.Setting[?]] = Def.settings(
    ...
    ScalafixConfig / products ++= Def.uncached {
      (
        scalaBinaryVersion.value match {
          case "2.12" =>
            (LocalProject("myScalafix2_12") / Compile / externalDependencyClasspath).value
          case _ =>
            (LocalProject("myScalafix2_13") / Compile / externalDependencyClasspath).value
        }
      ).map(x => fileConverter.value.toPath(x.data).toFile)
    }
  )
}
